...
- Kafka
  ...
  - key concept
    ...
    - event
      - message 被叫做 event
        - 有 key, value, timestamp
    - partition
      - topic 通常會有多個 partition
      - 通常 partition 的數量至少為 consumer 數量
      - 優點
        - 提高 throughput
      - 缺點
        - 過多的 paratition 會提高 latency
    - consumer group
      - 一個 consumer group 可以有多個 consumer
      - 只會收到 message 一次，並派給底下的某個 consumer
    - consumer offset
      - consumer 會記錄自己消耗到哪裡了，消耗完了就增加自己的 offset，移動到下一個
